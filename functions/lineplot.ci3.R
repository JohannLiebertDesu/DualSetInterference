...
lineplot.ci3 <- function(data, dv, iv, id=1, group=NULL, x=1, off=0, Bakeman=T, upper=T, lower=T, ylim=c(0,1), xlim=NULL, 
                         pt=15:25, lty=rep(1,11), col=rep("black", 11), ptcol=rep("white",11),na.rm=T, cex=1, 
                         xticks = NULL,  ...)  # default: remove NAs, so function runs over valid cases, without returning NA!
  
{
  library(Hmisc)
  
  data <- as.data.frame(data) # in case data is a tibble: aggregate does not work with tibbles!
  ...
  if (!is.numeric(Mdata[,iv[x]])) Mdata[,iv[x]] <- as.factor(Mdata[,iv[x]])
  ...
  if (dimensions==1) {
    ...
    if (!is.numeric(Mdata[,iv[3-x]])) Mdata[,iv[3-x]] <- as.factor(Mdata[,iv[3-x]])
    iv2vals <- sort(unique(as.numeric(Mdata[,iv[3-x]])))
    loop <- length(iv2vals)
  }
  
  if (length(pt) < loop) pt <- rep(pt, loop)
  if (length(ptcol) < loop) ptcol <- rep(ptcol, loop)
  if (length(col) < loop) col <- rep(col, loop)
  if (length(lty) < loop) lty <- rep(lty, loop)
  for (j in 1:loop)  {
    if (dimensions==1) {
      y <- Mdata$x
      dci <- Mdata$ci
    ...
    CIlow <- y - dci
    xx <- xx + (j-1)*off
    plot(xx, y, xlim=xlim, ylim=ylim, type="b", lty=lty[j], pch=pt[j], col=col[j], bg=ptcol[j], xaxt=xaxt, cex=cex, ...)  
    errbar(xx, y, yplus=CIhigh, yminus=CIlow, xlim=xlim, ylim=ylim,add=T,type="n", xaxt=xaxt, errbar.col = col[j], ...)
